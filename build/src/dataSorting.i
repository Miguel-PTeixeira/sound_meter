# 0 "src/dataSorting.c"
# 1 "/home/aluno/Desktop/sound_meter-master_v02//"
# 0 "<built-in>"
# 0 "<command-line>"
...
# 3 "src/dataSorting.c" 2
# 1 "src/dataSorting.h" 1
...
# 10 "src/dataSorting.h"
typedef struct DataL_node{
    struct DataL_node* next;
    float data;
} DataL_node;


DataL_node* create_data();
int add_element(DataL_node* d, void* value);
float get_percentil(DataL_node* d, int percentil);
void free_data(DataL_node* d);
# 4 "src/dataSorting.c" 2

void remove_first_elem(DataL_node* head);


DataL_node* create_data() {
    DataL_node* data = malloc(sizeof(DataL_node));
    if (!data) return 
# 10 "src/dataSorting.c" 3 4
                     ((void *)0)
# 10 "src/dataSorting.c"
                         ;
    data->next = 
# 11 "src/dataSorting.c" 3 4
                ((void *)0)
# 11 "src/dataSorting.c"
                    ;
    data->data = 0.0f;
    return data;
}
...
int add_element(DataL_node* head, void* value) {
    if (!head || !value) return 0;
    DataL_node* current = head;


    int size = 0;
    while (current->next != 
# 23 "src/dataSorting.c" 3 4
                           ((void *)0)
# 23 "src/dataSorting.c"
                               ) {
  size++;
    ...
    if(size>10){
  remove_first_elem(head);
 }


    DataL_node* new_node = malloc(sizeof(DataL_node));
    if (!new_node) return 0;

    new_node->data = *(float*)value;
    new_node->next = 
# 37 "src/dataSorting.c" 3 4
                    ((void *)0)
# 37 "src/dataSorting.c"
                        ;
...
float get_percentil(DataL_node* head, int percentil) {
    if (!head || percentil < 0 || percentil > 100) return 0;


    int count = 0;
    DataL_node* current = head->next;
    while (current) {
        count++;
        current = current->next;
    }
    ...
    qsort(array, count, sizeof(float), cmp_func);

    int index = (percentil * count) / 100;
...
void free_data(DataL_node* head) {
    DataL_node* current = head;
    while (current) {
        DataL_node* next = current->next;
        free(current);
        current = next;
    }
}
